...
function playEnemyDieSound(isPurple) {
    if (!audioCtx) return;
    let osc = audioCtx.createOscillator();
    let gain = audioCtx.createGain();
    osc.connect(gain);
    gain.connect(audioCtx.destination);
    osc.type = 'triangle';
    let duration = isPurple ? 0.25 : 0.12;
    let baseFreq = isPurple ? 180 : 220;
